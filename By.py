...
def bot(op):
    try:
            ...
            if msg.contentType == 13:
               if wait["wblack"] == True:
				...
				if msg.from_ in admin:
					...
					kk.sendMessage(msg)
            elif msg.text in ["忙鈥炩?好Ｂ?Ｆ???Ｆ捖ｂ?毬?Ｆ捖趁Ｆ??","Gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					...
					msg.text = None
					cl.sendMessage(msg)
            elif msg.text in ["忙鈥炩?好Ｂ?Ｆ???Ｆ捖ｂ?毬?Ｆ捖趁Ｆ??","Cv1 gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
					...
					msg.text = None
					ki.sendMessage(msg)
            elif msg.text in ["忙鈥炩?好Ｂ?Ｆ???Ｆ捖ｂ?毬?Ｆ捖趁Ｆ??","Cv2 gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
					...
					msg.text = None
					kk.sendMessage(msg)
            elif msg.text in ["忙鈥炩?好Ｂ?Ｆ???Ｆ捖ｂ?毬?Ｆ捖趁Ｆ??","Cv3 gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
					...
					msg.text = None
					kc.sendMessage(msg)
            elif msg.text in ["忙鈥炩?好Ｂ?Ｆ???Ｆ捖ｂ?毬?Ｆ捖趁Ｆ??","All gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
		...
		for ticket_id in n_links:
              ...
              if msg.from_ in admin:
                if wait["ProtectQR"] == False:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"done")
            elif msg.text in ["茅鈧Ｃ德???λ?:茫鈥毬Ｆ捖?","K on","Contact on","茅隆?莽?潞茂??茅鈥???"]:
				if msg.from_ in admin:
					if wait["contact"] == True:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"done")
            elif msg.text in ["茅鈧Ｃ德???λ?:茫鈥毬Ｆ???","K off","Contact off","茅隆?莽?潞茂??茅鈥??"]:
				if msg.from_ in admin:
					if wait["contact"] == False:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"done")
            elif msg.text in ["?鈥÷??光???????? :茫鈥毬Ｆ捖?","Join on","Auto join:on","?鈥÷??光???徠捗?? 茂??茅鈥???"]:
				if msg.from_ in admin:
					if wait["autoJoin"] == True:
						if wait["lang"] == "JP":
							cl.sendText(msg.to,"already on")
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"done")
            elif msg.text in ["?鈥÷??光???????? :茫鈥毬Ｆ???","Join off","Auto join:off","?鈥÷??光???徠捗?? 茂??茅鈥??"]:
				if msg.from_ in admin:
					...
					try:
						...
						if strnum == "off":
							...
							if wait["lang"] == "JP":
								...
								cl.sendText(msg.to,"氓鈥β趁?衡?犆??氣偓??路忙鈥光??宦?ｂ偓鈥?β?︹??睹?尖偓??路忙艗鈥????郝好︹??懊???樏?偓?")
						else:
							num =  int(strnum)
							...
							if wait["lang"] == "JP":
								...
								cl.sendText(msg.to,strnum + "盲??盲潞潞盲??盲?鈥姑♀???????????ㄢ?÷?犅??氣偓??路忙鈥光??宦?")
					except:
						if wait["lang"] == "JP":
							cl.sendText(msg.to,"Value is wrong")
						else:
							cl.sendText(msg.to,"Bizarre ratings")
            elif msg.text in ["氓?路氓?露?鈥÷??光???偓鈧??÷?:茫鈥毬Ｆ捖?","Leave on","Auto leave:on","氓?路氓?露?鈥÷??光???偓鈧??÷好寂??????"]:
				if msg.from_ in admin:
					if wait["leaveRoom"] == True:
						if wait["lang"] == "JP":
							cl.sendText(msg.to,"already on")
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"???盲潞鈥犆?尖偓茫鈧??")
            elif msg.text in ["氓?路氓?露?鈥÷??光???偓鈧??÷?:茫鈥毬Ｆ???","Leave off","Auto leave:off","氓?路氓?露?鈥÷??光???偓鈧??÷好寂?????"]:
				if msg.from_ in admin:
					if wait["leaveRoom"] == False:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"already")
            elif msg.text in ["氓鈥β泵ε???:茫鈥毬Ｆ捖?","Share on","Share on"]:
				if msg.from_ in admin:
					if wait["timeline"] == True:
						if wait["lang"] == "JP":
							cl.sendText(msg.to,"already on")
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"???盲潞鈥犆?尖偓茫鈧??")
            elif msg.text in ["氓鈥β泵ε???:茫鈥毬Ｆ???","Share off","Share off"]:
				if msg.from_ in admin:
					if wait["timeline"] == False:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"???盲潞鈥犆??β趁︹??ｂ偓鈥?")
				...
				if msg.from_ in admin:
					...
					if album["result"]["items"] == []:
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"莽鈥郝???犈捗β猜???ｂ偓鈥?")
					else:
						if wait["lang"] == "JP":
				...
				if msg.from_ in admin:
					...
					if wait["lang"] == "JP":
						...
						cl.sendText(msg.to,str(i) + "氓? 茅????衡?犆?衡?姑♀????郝???犈捗ｂ偓鈥?")
            elif msg.text in ["Group id","莽戮?莽碌鈥???β╥d"]:
				if msg.from_ in admin:
				...
				if msg.from_ in admin:
					...
					if wait["lang"] == "JP":
						...
						cl.sendText(msg.to,"忙鈥光??宦??衡?犆??β?捖♀????氣偓??路茫鈧??")
            elif "album remove芒鈥???" in msg.text:
				if msg.from_ in admin:
					gid = msg.text.replace("album remove芒鈥???","")
					albums = cl.getAlbum(gid)["result"]["items"]
					i = 0
					if albums != []:
						for album in albums:
					...
					if wait["lang"] == "JP":
						...
						cl.sendText(msg.to,str(i) + "氓? 茅????衡?犆?衡?姑♀????郝???犈捗ｂ偓鈥?")
            elif msg.text in ["?鈥÷??光??柯??? :茫鈥毬Ｆ捖?","Add on","Auto add:on","?鈥÷??光??柯??? 茂??茅鈥???"]:
				if msg.from_ in admin:
					if wait["autoAdd"] == True:
						if wait["lang"] == "JP":
							cl.sendText(msg.to,"already on")
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"???盲潞鈥犆?尖偓茫鈧??")
            elif msg.text in ["?鈥÷??光??柯??? :茫鈥毬Ｆ???","Add off","Auto add:off","?鈥÷??光??柯??? 茂??茅鈥??"]:
				if msg.from_ in admin:
            ...
            elif msg.text in ["Message","?鈥÷??光??柯??? 氓鈥???偓??久?好?"]:
				if msg.from_ in admin:
					if wait["lang"] == "JP":
				...
				if msg.from_ in admin:
					...
					if c in [""," ","\n",None]:
						...
						wait["comment"] = c
						cl.sendText(msg.to,"changed\n\n" + c)
            elif msg.text in ["茫鈥毬趁Ｆ捖?Ｆ捖趁Ｆ??:茫鈥毬Ｆ捖?","Comment on","Comment:on","?鈥÷??光???︹??? ?莽鈥?劉??鈧寂??????"]:
				if msg.from_ in admin:
					if wait["commentOn"] == True:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"???盲潞鈥犆?尖偓茫鈧??")
            elif msg.text in ["茫鈥毬趁Ｆ捖?Ｆ捖趁Ｆ??:茫鈥毬Ｆ???","Comment on","Comment off","?鈥÷??光???︹??? ?莽鈥?劉??鈧寂?????"]:
				if msg.from_ in admin:
					if wait["commentOn"] == False:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"???盲潞鈥犆??β趁︹??ｂ偓鈥?")
            elif msg.text in ["Comment","莽鈥?劉??鈧?好?"]:
				if msg.from_ in admin:
...
#-----------------------------------------------
				...
				if msg.from_ in admin:
					ki.sendText(msg.to,"Chomel pekok 魛?魛?Har Har?靠")
					kk.sendText(msg.to,"Chomel pekok 魛?魛?Har Har?靠")
					kc.sendText(msg.to,"Chomel pekok 魛?魛?Har Har?靠")
...
#-----------------------------------------------
				...
				ki.sendText(msg.to,"Mamanx 魛?魛?Har Har?靠")
				kk.sendText(msg.to,"Ngentod 魛?魛?Har Har?靠")
				kc.sendText(msg.to,"Yuuk 魛?魛?Har Har?靠")
#-----------------------------------------------
...
#------------------------------------------------------------------
				...
				if msg.from_ in admin:
					try:
						...
						cl.sendText(msg.to,"Error")
            elif "fakec芒鈥???" in msg.text:
				if msg.from_ in admin:
					try:
						source_str = 'abcdefghijklmnopqrstuvwxyz1234567890@:;./_][!&%$#)(=~^|'
						name = "".join([random.choice(source_str) for x in xrange(10)])
						anu = msg.text.replace("fakec芒鈥???","")
