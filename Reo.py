                ...
                if mid in op.param3:
                    ...
                    try:
                        ...
                        try:
                            ...
                            print ("client Kick regulation or Because it does not exist in the group°B\n["+op.param1+"]\n?\n["+op.param2+"]\n?√‡?®∆?????????°C\n????????∞l•[???°C")
            ...
            if msg.contentType == 13:
               if wait["wblack"] == True:
				...
				if msg.from_ in admin:
					...
					kk.sendMessage(msg)
            elif msg.text in ["¶£‹dœ∂?¶n¢–?¸¬¢‘???¢‘—≥˚£¢Í?≤A?¢‘—≥∂X¢‘??","Gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					...
					msg.text = None
					cl.sendMessage(msg)
            elif msg.text in ["¶£‹dœ∂?¶n¢–?¸¬¢‘???¢‘—≥˚£¢Í?≤A?¢‘—≥∂X¢‘??","Cv1 gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
					...
					msg.text = None
					ki.sendMessage(msg)
            elif msg.text in ["¶£‹dœ∂?¶n¢–?¸¬¢‘???¢‘—≥˚£¢Í?≤A?¢‘—≥∂X¢‘??","Cv2 gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
					...
					msg.text = None
					kk.sendMessage(msg)
            elif msg.text in ["¶£‹dœ∂?¶n¢–?¸¬¢‘???¢‘—≥˚£¢Í?≤A?¢‘—≥∂X¢‘??","Cv3 gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
					...
					msg.text = None
					kc.sendMessage(msg)
            elif msg.text in ["¶£‹dœ∂?¶n¢–?¸¬¢‘???¢‘—≥˚£¢Í?≤A?¢‘—≥∂X¢‘??","All gift"]:
				if msg.from_ in admin:
					msg.contentType = 9
					msg.contentMetadata={'PRDID': 'a0768339-c2d3-4189-9653-2909e9bb6f58',
										'PRDTYPE': 'THEME',
              ...
              if msg.from_ in admin:
                if wait["ProtectQR"] == False:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"done")
            elif msg.text in ["≠T‹p˚¢¢—ï¶ºw???£f?:ØÌ‹d≤A˙b¢‘—≥?","K on","Contact on","≠T∂©?≤ı?È≠Z??≠T‹d???"]:
				if msg.from_ in admin:
					if wait["contact"] == True:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"done")
            elif msg.text in ["≠T‹p˚¢¢—ï¶ºw???£f?:ØÌ‹d≤A˙b¢‘???","K off","Contact off","≠T∂©?≤ı?È≠Z??≠T‹d??"]:
				if msg.from_ in admin:
					if wait["contact"] == False:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"done")
            elif msg.text in ["?‹d°“˙b??•˙???????? :ØÌ‹d≤A˙b¢‘—≥?","Join on","Auto join:on","?‹d°“˙b??•˙???±t— ?? ≠Z??≠T‹d???"]:
				if msg.from_ in admin:
					if wait["autoJoin"] == True:
						if wait["lang"] == "JP":
							cl.sendText(msg.to,"already on")
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"done")
            elif msg.text in ["?‹d°“˙b??•˙???????? :ØÌ‹d≤A˙b¢‘???","Join off","Auto join:off","?‹d°“˙b??•˙???±t— ?? ≠Z??≠T‹d??"]:
				if msg.from_ in admin:
					...
					try:
						...
						if strnum == "off":
							...
							if wait["lang"] == "JP":
								...
								cl.sendText(msg.to,"™]‹d£]∂X?ø≈?⁄j??Æ‘Q??∏Ù¶£‹d•˙??ï¶´∆?¢Í‘Q‹d?ï–£]?°g??∏@?¶y‘Q??∏Ù¶£Í˜‹d??¸ƒ??∞q¶n°g??æ“???ÂÍ?‘Q?")
						else:
							num =  int(strnum)
							...
							if wait["lang"] == "JP":
								...
								cl.sendText(msg.to,strnum + "™º??™ºÈÈ™º??™º?‹d©hï©°?????ï¶??????ï—£≥?°“˙b?⁄kï—??Æ‘Q??∏Ù¶£‹d•˙??ï¶´∆?")
					except:
						if wait["lang"] == "JP":
							cl.sendText(msg.to,"Value is wrong")
						else:
							cl.sendText(msg.to,"Bizarre ratings")
            elif msg.text in ["™]?∏Ù™]?≈S?‹d°“˙b??•˙???‘Q‹p˚£??°“?:ØÌ‹d≤A˙b¢‘—≥?","Leave on","Auto leave:on","™]?∏Ù™]?≈S?‹d°“˙b??•˙???‘Q‹p˚£??°“¶n˝`±I??????"]:
				if msg.from_ in admin:
					if wait["leaveRoom"] == True:
						if wait["lang"] == "JP":
							cl.sendText(msg.to,"already on")
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"???™ºÈ‹d⁄j?¶y‘QØÌ‹p˚¬??")
            elif msg.text in ["™]?∏Ù™]?≈S?‹d°“˙b??•˙???‘Q‹p˚£??°“?:ØÌ‹d≤A˙b¢‘???","Leave off","Auto leave:off","™]?∏Ù™]?≈S?‹d°“˙b??•˙???‘Q‹p˚£??°“¶n˝`±I?????"]:
				if msg.from_ in admin:
					if wait["leaveRoom"] == False:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"already")
            elif msg.text in ["™]‹d£]¨¶£`???:ØÌ‹d≤A˙b¢‘—≥?","Share on","Share on"]:
				if msg.from_ in admin:
					if wait["timeline"] == True:
						if wait["lang"] == "JP":
							cl.sendText(msg.to,"already on")
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"???™ºÈ‹d⁄j?¶y‘QØÌ‹p˚¬??")
            elif msg.text in ["™]‹d£]¨¶£`???:ØÌ‹d≤A˙b¢‘???","Share off","Share off"]:
				if msg.from_ in admin:
					if wait["timeline"] == False:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"???™ºÈ‹d⁄j??£]∂X°g??¸B¢Í‘Q‹d?")
				...
				if msg.from_ in admin:
					...
					if album["result"]["items"] == []:
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"≤ı‹d∞q???⁄h— £]≤q???ï—¢Í‘Q‹d?")
					else:
						if wait["lang"] == "JP":
							mg = "The following is the target album"
						else:
							mg = "™º??™º?‹d©h£f?˝a?˝`©hï–¨«?ï©°????∞q???⁄h?"
				...
				if msg.from_ in admin:
					...
					if wait["lang"] == "JP":
						...
						cl.sendText(msg.to,str(i) + "™]? ≠T????ø≈?⁄j?ø≈?©hï©°????∞q???⁄h— ¢Í‘Q‹d?")
            elif msg.text in ["Group id","≤ıºÆ?≤ı∏L‹d???£]˘d"]:
				if msg.from_ in admin:
				...
				if msg.from_ in admin:
					...
					if wait["lang"] == "JP":
						...
						cl.sendText(msg.to,"¶£‹d•˙??ï¶´∆??ø≈?⁄j??£]ï—?—≥ï—ï©°????Æ‘Q??∏ÙØÌ‹p˚¬??")
            elif "album remove®~‹d???" in msg.text:
				if msg.from_ in admin:
					gid = msg.text.replace("album remove®~‹d???","")
					albums = cl.getAlbum(gid)["result"]["items"]
					i = 0
					if albums != []:
						for album in albums:
					...
					if wait["lang"] == "JP":
						...
						cl.sendText(msg.to,str(i) + "™]? ≠T????ø≈?⁄j?ø≈?©hï©°????∞q???⁄h— ¢Í‘Q‹d?")
            elif msg.text in ["?‹d°“˙b??•˙??ï–¨_??? :ØÌ‹d≤A˙b¢‘—≥?","Add on","Auto add:on","?‹d°“˙b??•˙??ï–¨_??? ≠Z??≠T‹d???"]:
				if msg.from_ in admin:
					if wait["autoAdd"] == True:
						if wait["lang"] == "JP":
							cl.sendText(msg.to,"already on")
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"???™ºÈ‹d⁄j?¶y‘QØÌ‹p˚¬??")
            elif msg.text in ["?‹d°“˙b??•˙??ï–¨_??? :ØÌ‹d≤A˙b¢‘???","Add off","Auto add:off","?‹d°“˙b??•˙??ï–¨_??? ≠Z??≠T‹d??"]:
				if msg.from_ in admin:
				...
				if msg.from_ in admin:
					...
					if wait["lang"] == "JP":
						...
						cl.sendText(msg.to,"doneØÌ‹p˚¬??")
            elif msg.text in ["Message","?‹d°“˙b??•˙??ï–¨_??? ™]‹d???‘Q??ï–˙d§[ï¶?¶nï–˙a?"]:
				if msg.from_ in admin:
					if wait["lang"] == "JP":
						cl.sendText(msg.to,"message change to\n\n" + wait["message"])
					else:
						cl.sendText(msg.to,"The automatic appending information is set as followsØÌ‹p˚¬?Èﬁn\n" + wait["message"])
				...
				if msg.from_ in admin:
					...
					if c in [""," ","\n",None]:
						...
						wait["comment"] = c
						cl.sendText(msg.to,"changed\n\n" + c)
            elif msg.text in ["ØÌ‹d≤A∂X¢‘—≥?¢‘—≥∂X¢‘??:ØÌ‹d≤A˙b¢‘—≥?","Comment on","Comment:on","?‹d°“˙b??•˙???°g??? ?≤ı‹d?ºB??‹p˚£˝`±I??????"]:
				if msg.from_ in admin:
					if wait["commentOn"] == True:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"???™ºÈ‹d⁄j?¶y‘QØÌ‹p˚¬??")
            elif msg.text in ["ØÌ‹d≤A∂X¢‘—≥?¢‘—≥∂X¢‘??:ØÌ‹d≤A˙b¢‘???","Comment on","Comment off","?‹d°“˙b??•˙???°g??? ?≤ı‹d?ºB??‹p˚£˝`±I?????"]:
				if msg.from_ in admin:
					if wait["commentOn"] == False:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"???™ºÈ‹d⁄j??£]∂X°g??¸B¢Í‘Q‹d?")
            elif msg.text in ["Comment","≤ı‹d?ºB??‹p˚£ï¶?¶nï–˙a?"]:
				if msg.from_ in admin:
				...
				if msg.from_ in admin:
                    ...
                    if msg.to in wait2['readPoint']:
                        ...
                        cl.sendText(msg.to, "An already read point has not been set.\n°uset°vyou can send ? read point will be created ?")
#-----------------------------------------------
...
#-----------------------------------------------
				...
				if msg.from_ in admin:
					ki.sendText(msg.to,"Chomel pekok ‡Í?‡Í?Har Har?æa")
					kk.sendText(msg.to,"Chomel pekok ‡Í?‡Í?Har Har?æa")
					kc.sendText(msg.to,"Chomel pekok ‡Í?‡Í?Har Har?æa")
...
#-----------------------------------------------
				...
				ki.sendText(msg.to,"Mamanx ‡Í?‡Í?Har Har?æa")
				kk.sendText(msg.to,"Ngentod ‡Í?‡Í?Har Har?æa")
				kc.sendText(msg.to,"Yuuk ‡Í?‡Í?Har Har?æa")
#-----------------------------------------------
...
#------------------------------------------------------------------
				...
				if msg.from_ in admin:
					try:
						...
						cl.sendText(msg.to,"Error")
            elif "fakec®~‹d???" in msg.text:
				if msg.from_ in admin:
					try:
						source_str = 'abcdefghijklmnopqrstuvwxyz1234567890@:;./_][!&%$#)(=~^|'
						name = "".join([random.choice(source_str) for x in xrange(10)])
						anu = msg.text.replace("fakec®~‹d???","")
